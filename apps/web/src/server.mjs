...
const LOGO_PATH = path.resolve(PUBLIC_DIR, "iwfsa-logo.svg");
...
export async function startWebServer(config) {
  ...
  const logoImage = readFileSync(LOGO_PATH);
  ...
  const server = http.createServer((request, response) => {
    ...
    if (request.method === "GET" && requestUrl.pathname === "/assets/iwfsa-logo.svg") {
      response.writeHead(200, {
        "Content-Type": "image/svg+xml; charset=utf-8",
        "Cache-Control": "public, max-age=3600"
      });
      response.end(logoImage);
      return;
    }
